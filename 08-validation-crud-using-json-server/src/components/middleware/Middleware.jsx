...
const randomId = () => {
    return Math.floor( Math.random() * 1000); // Id of the random user
}
...
const MiddleWare = () => {
    ...
    const readValue = (e) => {
        ...
        validate(e, name, value);  //e.target.value
        setContact({...contact, [name]: value }); // Update State 
    }
    ...
    const validate = (event, name, value) => {
        switch (name){
            ...
            case "name":
                if(value.length === 0){
                    errorPrint(name, "Please Enter the name...");
    ...
    const submitData = async (e) => {
        ...
        if(Object.keys(errors).length === 0  && Object.keys(contact).length !== 0){
            ...
            contact.id = randomId(); // Assigning a random id to the contact object
            await axios.post('/contacts', contact).then(res =>{ // Send
                toast.success("New User Created"); // Success Message
                navigate(`/`); // Redirecting to Home Page
            }).catch((error)=> toast.error(error.message)); // Error Message
        } else {
            toast.error("Please Fill All Fields Properly"); // Error Message
            // Reset Form
            setContact(initState); // Reset Contact
            setErrors({}); // Reset Errors Message
        }
    }
